...
router.get('', async (req, res) => {
    try {
        ...
        console.error("Error en GET /empresa:", e.message);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        console.error(`Error en GET /empresa/${req.params.id}:`, e.message);
        res.status(500).send(`Error: ${e.message}`); 
    }
});
...
router.post('', async (req, res) => {
    try {
        ...
        data != null ? res.status(201).json(data) : res.status(500).send('Error interno al crear la empresa.');
    } catch (e) { 
        console.error("Error en POST /empresa:", e.message);
        res.status(500).send(`Error: ${e.message}`); 
    }
});
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (data !== null) {
            ...
            res.status(404).send('No encontrado: La empresa con ese ID no existe.');
        }
    } catch (e) {
        console.error(`Error en PUT /empresa/${req.params.id}:`, e.message);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        ok ? res.status(200).send('Eliminado exitosamente.') : res.status(404).send('No encontrado: La empresa con ese ID no existe.');
    } catch (e) { 
        console.error(`Error en DELETE /empresa/${req.params.id}:`, e.message);
        res.status(500).send(`Error: ${e.message}`); 
    }
});
